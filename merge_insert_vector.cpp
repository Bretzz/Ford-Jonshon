...
static void	move_cell(std::vector<unsigned int>& dest, std::vector<unsigned int>& src, int pos, size_t size)
{
...
the pend */
static void	push_pend(std::vector<unsigned int>& main, std::vector<unsigned int>& pend, size_t size)
{
	size_t i = (3 * size) - 1;
	while (i <= main.size() - 1)
	{
...
static int	binary_split(const std::vector<unsigned int>& main, size_t low, size_t high, unsigned int target, size_t size)
{
