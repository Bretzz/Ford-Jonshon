...
static int insert_part_of_the_sort(std::deque<unsigned int>& main, std::deque<unsigned int>::iterator cell, size_t bound, size_t size)
{
	if (bound > main.size())
...
void	merge_insert(std::deque<unsigned int>& main, size_t size)
{
	...
	for (int j = 3; j < 34; ++j)
	{
		for (ssize_t i = (jacobsthal[j] - 1) * size - size; i > last; i -= size)
		{
			...
			if (insert_part_of_the_sort(main, pend.begin() + i, (std::pow(2, j - 1) - 1) * size - 1, size))
				return ;
		}
		if (jacob_cap == 1)
			return ;
